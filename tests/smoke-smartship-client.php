...
$GLOBALS['webbership_ss_http']         = null; // canned response for the next call
$GLOBALS['webbership_ss_last_request'] = null;
...
function wp_remote_request( $url, $args = [] ) {
  $GLOBALS['webbership_ss_last_request'] = [ 'url' => $url, 'args' => $args ];
  return $GLOBALS['webbership_ss_http'];
}
...
$req = $GLOBALS['webbership_ss_last_request'];
...
assert_true( isset( $GLOBALS['webbership_ss_last_request']['args']['headers']['X-Shop-Url'] ), 'shop headers on /cost' );
$client->request( 'GET', '/geolocation/counties' );
assert_true( ! isset( $GLOBALS['webbership_ss_last_request']['args']['headers']['X-Shop-Url'] ), 'no shop headers elsewhere' );
...
$client->get_cities( 7 );
assert_true( strpos( $GLOBALS['webbership_ss_last_request']['url'], 'county=7' ) !== false, 'cities: county id' );
...
assert_same( 7, $GLOBALS['webbership_ss_last_request']['args']['timeout'], 'counties: custom timeout' );
ss_set_response( 200, [ 'status' => 200, 'senders' => [] ] );
...
assert_true( isset( $GLOBALS['webbership_ss_last_request']['args']['headers']['X-Shop-Url'] ), 'cost: shop headers' );
// cost() honors a custom timeout (RATE_TIMEOUT at checkout).
ss_set_response( 200, [ 'status' => 200, 'costs' => [] ] );
$client->cost( [ 'recipient' => [], 'sender' => [], 'content' => [] ], 3 );
assert_same( 3, $GLOBALS['webbership_ss_last_request']['args']['timeout'], 'cost: custom timeout' );
ss_set_response( 200, [ 'status' => 200, 'awb' => 'AWB1' ] );
$client->create_awb( [ 'recipient' => [], 'sender' => [], 'content' => [], 'courier_id' => 2 ] );
assert_true( ! isset( $GLOBALS['webbership_ss_last_request']['args']['headers']['X-Shop-Url'] ), 'create_awb: no shop headers' );
...
assert_true( strpos( $GLOBALS['webbership_ss_last_request']['url'], '/awb/status/A%20B%2F2' ) !== false, 'status: rawurlencoded' );
...
assert_true( strpos( $GLOBALS['webbership_ss_last_request']['url'], '/awb/print/AWB1/A6' ) !== false, 'print: url+format' );
...
assert_true( strpos( $GLOBALS['webbership_ss_last_request']['url'], '/awb/print/AWB1/A4' ) !== false, 'print: bad format -> A4' );
...
assert_true( strpos( $GLOBALS['webbership_ss_last_request']['url'], '/awb/cancel/AWB1' ) !== false, 'cancel: url' );
