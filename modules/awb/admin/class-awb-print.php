...
final class AwbPrint {
  ...
  private const ACTION = 'webbership_smartship_print';
  ...
  public function handle(): void {
    if ( ! current_user_can( self::CAP ) ) { wp_die( esc_html__( 'Forbidden.', 'webbership-smartship' ) ); }
    check_admin_referer( self::ACTION );
    $order_id = isset( $_GET['order_id'] ) ? absint( $_GET['order_id'] ) : 0;
    $format   = ( isset( $_GET['format'] ) && 'A6' === $_GET['format'] ) ? 'A6' : 'A4';
    $order    = $order_id ? wc_get_order( $order_id ) : false;
    if ( ! $order ) { wp_die( esc_html__( 'Order not found.', 'webbership-smartship' ) ); }
    $awb = (string) $order->get_meta( '_webbership_smartship_awb' );
    if ( '' === $awb ) { wp_die( esc_html__( 'No AWB on this order.', 'webbership-smartship' ) ); }

    $res = ( new SmartShipClient( Settings::api_key() ) )->print_awb( $awb, $format );
    if ( empty( $res['ok'] ) || empty( $res['pdf'] ) ) {
      wp_die( esc_html( $res['message'] ?: __( 'Could not fetch the label.', 'webbership-smartship' ) ) );
    }
    if ( strncmp( (string) $res['pdf'], '%PDF', 4 ) !== 0 ) {
      wp_die( esc_html( $res['message'] ?: __( 'SmartShip did not return a valid PDF.', 'webbership-smartship' ) ) );
    }
    nocache_headers();
